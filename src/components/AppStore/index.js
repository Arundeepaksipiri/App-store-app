...
class AppStore extends Component {
  state = {activateId: tabsList[0].tabId, searchValue: ''}
  onFilterApps = () => {
    const {activateId, searchValue} = this.state
    const filterList = appsList.filter(
      eachItem =>
        eachItem.category === activateId &&
        eachItem.appName.toLowerCase().includes(searchValue.toLowerCase()),
    )
    return filterList
  }
  onChangeValue = event => {
  ...
  onSelectionTab = id => {
    this.setState({activateId: id})
  }
  render() {
    const {activateId, searchValue} = this.state
    const filterList = this.onFilterApps()
    return (
        ...
        <ul className="tab-items">
          {tabsList.map(eachTab => (
            <TabItem
              Tab={eachTab}
              key={eachTab.tabId}
              tabSelect={this.onSelectionTab}
              isActivated={eachTab.tabId === activateId}
            />
          ))}
        </ul>
